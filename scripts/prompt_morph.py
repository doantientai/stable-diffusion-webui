...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        i1 = gr.HTML(
            '<p style="margin-bottom:0.75em">Keyframe Format: <br>Seed | Prompt or just Prompt</p>'
        )
        prompt_list = gr.TextArea(
            label="Prompt list",
            placeholder="Enter one prompt per line. Blank lines will be ignored.",
        )
        n_images = gr.Slider(
            minimum=2,
            maximum=256,
            value=25,
            step=1,
            label="Number of images between keyframes",
        )
    ...
    def run(self, p, i1, prompt_list, n_images, save_video, video_fps):
        ...
        for n in range(1, len(prompts)):
            ...
            (
                ...
                prompt_flat_list,
                prompt_indexes,
            ) = prompt_parser.get_multicond_prompt_list([start_prompt, target_prompt])
        ...
        if save_video:
            clip = ImageSequenceClip.ImageSequenceClip(
                [np.asarray(t) for t in all_images], fps=video_fps
            )
            clip.write_videofile(
                os.path.join(morph_path, f"morph-{morph_number:05}.webm"),
                codec="libvpx-vp9",
        ...
        if opts.return_grid:
            ...
            if opts.grid_save:
                images.save_image(
                    ...
                    p.outpath_grids,
                    "grid",
                    processed.all_seeds[0],
                    processed.prompt,
                    opts.grid_format,
                    info=processed.infotext(p, 0),
